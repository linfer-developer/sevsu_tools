...
from src.pysevsu.web import Parser
from src.pysevsu.xls import ExcelFile, Worksheet
...
class BatchCTEExporter:
    def __init__(
        ...
        self._weeks_buffer: Set[Any] = set()
        self._groups_buffer: Set[Any] = set()
        self._lessons_buffer: Set[Any] = set()
    ...
    @staticmethod
    def _get_lesson(
        week_temp_key: str, group_temp_key: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        return {
    ...
    async def add(self, data: Dict[str, Any]) -> None:
        async with self._lock:
            ...
            if week_temp_key not in self._cache:
                self._weeks_buffer.add(week_data)
                self._cache.add(week_temp_key)

            if group_temp_key not in self._cache:
                self._groups_buffer.add(group_data)
                self._cache.add(group_temp_key)

            lesson_data = self._get_lesson(
                week_temp_key=week_temp_key, group_temp_key=group_temp_key, data=data
            )
            self._lessons_buffer.add(lesson_data)
    ...
    async def _flush_buffered_data(self) -> None:
        ...
        async with self._semaphore:
            await self._execute_cte_insertion(
                weeks_to_insert, groups_to_insert, lessons_to_insert
            )
    ...
    async def _execute_cte_insertion(
        ...
        insert_query = self._build_final_insert_query(week_cte, group_cte, lessons_data)
    ...
    def _build_final_insert_query(
        self, week_cte: str, group_cte: str, lessons_data: List[Dict[str, Any]]
    ) -> str:
        lesson_values = []
...
class Session:
    def __init__(
        ...
        self._exporter = BatchCTEExporter(
...
<<<<<<< HEAD:lib/pysevsu/schedule/engine/worker.py
            web = Parser()
            async for button_data in web.run_data_stream():
                ...
                tasks.append(task)
=======
            async for data in Parser().run_data_stream():
                task = asyncio.create_task(self._run_xls_files_headler(data.copy()))
                tasks.append(asyncio.create_task(task))
>>>>>>> 7a25e39e5d7ca94dd43466258cfd0809a3508147:src/pysevsu/core/_legasy.py
    ...
    async def _run_xls_files_headler(self, data: Dict[Any, Any]) -> None:
        ...
        async for sheet in xls.run_worksheets_stream():
            ...
            task = self._run_worksheet_hander(sheet, data.copy())
            tasks.append(asyncio.create_task(task))

        await asyncio.gather(*tasks)

    async def _run_worksheet_hander(
        self, xls_sheet: Worksheet, data: Dict[Any, Any]
    ) -> None:
